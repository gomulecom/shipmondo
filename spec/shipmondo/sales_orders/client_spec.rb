...
describe Shipmondo::SalesOrders::Client do
  ...
  describe :create do
    it 'can create the order' do
      create_request = stub_request(:post, 'https://app.shipmondo.com/api/public/v3/sales_orders').with(body: File.read('spec/fixtures/create_sales_order.json').strip)

      sales_order = Shipmondo::SalesOrders::SalesOrder.new(
        order_id: '27000',
